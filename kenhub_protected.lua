-- KenHub license client. Replace API_URL before use.
local API_URL = "https://YOUR-DOMAIN.example"
local KEY_FILE = "kenhub_license.txt"
...
local function getHWID()
    ...
    if not ok or not id or id == "" then error("Unable to obtain a device identifier") end
    return id
end
...
local function validate(key)
    ...
    if not req then return false, "HTTP request function unavailable" end
    local body = game:GetService("HttpService"):JSONEncode({key=key, hwid=getHWID()})
    local ok, response = pcall(req, {Url=API_URL.."/api/validate", Method="POST", Headers={['Content-Type']='application/json'}, Body=body})
    ...
    if status < 200 or status >= 300 or not data.ok then return false, data.error or "Authentication failed" end
    return true, data
end
...
local function authenticate()
    local key = readKey()
    if not key then
        error("No license key found. Set your key in the loader/key file.")
    end
    local ok, data = validate(key)
    if not ok then error("KenHub authentication failed: "..tostring(data)) end
    if data.plan == "free" then
        warn("KenHub: Free license active")
    elseif data.plan == "premium" then
        warn("KenHub: Premium license active")
    elseif data.plan == "lifetime" then
        warn("KenHub: Lifetime license active")
    end
    return data
end

local _kenhubLicense = authenticate()
